...
public class ClienteController {
    ...
    @GetMapping("/{id}")
    public Cliente findCountryById(@PathVariable Integer id) {
    ...
    @GetMapping("/")
    public Cliente findByLike(@RequestParam("nome") String nome) {
        Optional<Cliente> optionalClient= clienteService.findByLike(nome);
        if (optionalClient.isPresent())
            return optionalClient.get();
